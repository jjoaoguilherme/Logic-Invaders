...
const Game = {
    ...
    PHASE_UPGRADES: {
        ...
        3: {
            ...
            description: "Armazena uma correção automática contra uma dedução errada na fase final.",
    ...
    _completeCampaign() {
        if (this.assistMode) {
            this._log("Campanha concluída no modo assistido; ranking não foi alterado.", "info");
        } else {
            this._addRankingEntry();
        }
        this.state = "gameover";
        this._setSentenceObscured(false);
    ...
    _checkPowerupPickup() {
        ...
        for (const p of this.powerups.powerups) {
            ...
            // NOVO: power-up especial abre mini-jogo; recompensa só é decidida após resposta.
            if (p.isSpecial) {
                this._startUpgradeChallenge(p);
                continue;
            }
    ...
    _saveRanking() {
        try {
            localStorage.setItem(
                ...
                JSON.stringify(this.rankingEntries.slice(0, this.MAX_RANKING_ENTRIES))
            );
        } catch (err) {
    ...
    _addRankingEntry() {
        ...
        this.rankingEntries.sort((a, b) => b.score - a.score);
        this.rankingEntries = this.rankingEntries.slice(0, this.MAX_RANKING_ENTRIES);
        this._saveRanking();
    },
    ...
    _drawMenu(ctx) {
        ...
        ctx.font      = '13px "Courier New", monospace';
        ctx.textAlign = "center";
        ctx.fillText("campanha fechada: níveis 1 a 3 · assistido sinaliza alvos e não ranqueia", w / 2, h / 2 + 4);
        ...
        ctx.font      = '12px "Courier New", monospace';
        ctx.textAlign = "center";
        ctx.fillText("use ↑ ↓ + ENTER  ou  CLIQUE  ·  ESC fecha telas", w / 2, h - 32);
        ctx.restore();
    },

    _drawRankingPreview(ctx) {
        const entries = this.rankingEntries.slice(0, 3);
        ...
        if (entries.length === 0) {
            ...
            ctx.fillText("Nenhuma campanha concluída.", x + 14, y + 60);
        } else {
            entries.forEach((entry, index) => {
                ...
                ctx.fillStyle = "#00D9FF";
                ctx.textAlign = "right";
                ctx.fillText(`${entry.score} pts`, x + w - 14, lineY);
                ctx.textAlign = "left";
            });
        }
        ctx.restore();
    ...
    _drawRankingScreen(ctx) {
        ...
        ctx.font      = '14px "Courier New", monospace';
        ctx.fillStyle = "rgba(230, 237, 247, 0.72)";
        ctx.fillText("Entram apenas pilotos que concluíram os 3 níveis.", 60, 112);

        const tableX = 80;
        const tableY = 150;
        const tableW = w - 160;
        const rowH   = 46;
        const entries = this.rankingEntries.slice(0, this.MAX_RANKING_ENTRIES);
        ...
        ctx.fillText("NÍVEL",  tableX + 560, tableY + 29);
        ctx.fillText("PTS",    tableX + 690, tableY + 29);
        ctx.fillText("DATA",   tableX + 810, tableY + 29);

        if (entries.length === 0) {
            ctx.fillStyle = "rgba(230, 237, 247, 0.65)";
            ctx.font      = '15px "Courier New", monospace';
            ctx.fillText("Nenhuma campanha concluída ainda.", tableX + 20, tableY + 88);
        } else {
            entries.forEach((entry, index) => {
                ...
                ctx.fillStyle = "#00FFCC";
                ctx.fillText(`${entry.level}`,  tableX + 560, rowY + 29);
                ctx.fillStyle = "#00D9FF";
                ctx.fillText(`${entry.score}`,  tableX + 690, rowY + 29);
                ctx.fillStyle = "rgba(230, 237, 247, 0.76)";
                ctx.fillText(entry.date || "-", tableX + 810, rowY + 29);
            });
        }
        ctx.restore();
    ...
    _drawRulesScreen(ctx) {
        ...
        const rules = [
            ...
            "• A campanha termina ao concluir o nível 3.",
            ...
            "• Item PERM: escolha um pacote A/B/C verdadeiro; erro/tempo/ESC vira temporário.",
            "• Só entra no ranking quem completar os 3 níveis.",
        ...
        ctx.fillStyle = "#00FFCC";
        ctx.font      = 'bold 18px "Courier New", monospace';
        ctx.fillText("UPGRADES FIXOS POR FASE:", rightX, ry);
        ry += 30;
    ...
    _drawControlsScreen(ctx) {
        ...
        ctx.fillStyle = "rgba(230, 237, 247, 0.55)";
        ctx.font      = '13px "Courier New", monospace';
        ctx.fillText("Dica: o item PERM de cada fase abre um desafio lógico antes de conceder efeito permanente.", 80, y + 10);
        ctx.restore();
